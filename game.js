import {initialState,transition,cityComplete,remainingAt,locationStatus,isPlaying} from './engine.js?v=0.2';
import {LOCATIONS,CITY_IDS,PALACE_IDS} from './data/locations.js?v=0.2';
import {OPENING,ENCOUNTERS,SCATTER,hintFor} from './data/dialogue.js?v=0.2';
import {ENDINGS} from './data/endings.js?v=0.2';

import {speakerIcon} from './data/characters.js?v=0.2';
...
const brand=()=>`<header class="brandbar"><div class="wordmark">${icon('star')}<span>DOTTOMON <b>RECALL</b></span></div><span class="edition">MARINA AU <i>·</i> 0.2</span></header>`;
function hud() {
...
function encounter() {
  ...
  let choices='';
  if (remaining.length) {
    choices=`<div class="capture-panel"><div class="capture-title"><span>CHOOSE YOUR APPROACH</span><small>${remaining.length} ${remaining.length===1?'assistant':'assistants'} here</small></div><div class="capture-buttons">${button('<b>Safe capture</b><span>33% success · 15s on success</span>','capture','button button--primary','data-method="safe"')}${button('<b>Risky capture</b><span>Guaranteed · 75s · 1 cigarette</span>','capture','button button--risk','data-method="risky"')}</div><p>Safe failure: 30s and +50% stress. Risky capture fills the stress bar immediately: 15s capture + a 60s smoking break.</p></div>`;
  }
...
function resultScreen() {
  ...
  const successText=r.method==='risky'&&story.riskySuccess?story.riskySuccess:story.success;
  const text=r.success?(Array.isArray(successText)?successText[r.target==='tea-one'?0:1]:successText):story.failures[(state.attempts[state.location]-1)%story.failures.length];
...
function endingScreen() {
  ...
  if(endingStep<3) {
    ...
    return `<main id="main" class="ending-page ending--${ending.color} page" tabindex="-1"><div class="ending-scene"><p class="eyebrow">${ending.scene}</p><div class="ending-beats" aria-label="Scene ${endingStep+1} of 3">${[0,1,2].map(n=>`<i class="${n<=endingStep?'lit':''}"></i>`).join('')}</div><h1>${ending.chapters[endingStep]}</h1><article class="prose">${lines(chapter)}</article>${button(endingStep===2?'See recall report ↗':'Continue →','ending-next','button button--primary')}<p class="reading-note">The search is over. Take your time.</p></div></main>`;
  }
  return `<main id="main" class="ending-page ending--${ending.color} page" tabindex="-1"><div class="ending-heading"><span class="ending-seal">${ending.mark}</span><p class="eyebrow">${ending.label}</p><h1>${ending.title}</h1><div class="ending-tagline">${ending.tagline.map(x=>`<p>${x}</p>`).join('')}</div></div><aside class="run-summary final-summary"><p class="eyebrow">RECALL REPORT</p><dl><div><dt>Time remaining</dt><dd>${time(summary.timeRemaining)}</dd></div><div><dt>Recovered during search</dt><dd>${summary.recovered} <small>/ 7</small></dd></div><div><dt>Cigarettes smoked</dt><dd>${summary.cigarettes}</dd></div></dl><p class="summary-outcome">${state.ending==='secret'?'All twenty are home. The seven escapees returned voluntarily.':state.ending==='good'?'All twenty are home. The laboratory stayed quiet.':state.ending==='breach'?`${summary.uncapturedPalace.length} uncaptured Palace assistant${summary.uncapturedPalace.length===1?'':'s'} reached the laboratory.`:'Zandik collected the remaining fugitives himself.'}</p>${button('Play again ↗','reset','button button--primary')}<p class="reading-note">A fresh search. Everything resets.</p></aside></main>`;
}
